...
extension Certificate.PrivateKey {
    ...
    struct SecKeyWrapper: Sendable {
        ...
        static func publicKeyData(privateKey: SecKey) throws -> Data {
            ...
            guard let publicSecKey = SecKeyCopyPublicKey(privateKey), let publicKeyData = SecKeyCopyExternalRepresentation(publicSecKey, &error) as Data? else {
                ...
                throw CertificateError.unsupportedPrivateKey(reason: "SecKeyCopyExternalRepresentation returned empty data")
            }
        ...
        static func signatureData<Bytes: DataProtocol>(key: SecKey, type: KeyType, digestAlgorithm: AlgorithmIdentifier, bytes: Bytes) throws -> Data {
            ...
            guard let signatureData = SecKeyCreateSignature(key,
                                                            ...
                                                            &error) as Data? else {
                if let error = error?.takeRetainedValue() {
                    throw CertificateError.unsupportedPrivateKey(reason: "could not create signature with SecKey: \(error)")
                }
